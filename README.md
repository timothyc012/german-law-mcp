...
38개 도구로 법령 조문, 법률 목차, 판례, 변호사 수임료, 기한 계산, 법적 감정서, 독일-EU법 비교·EUR-Lex 직접 조회, 위임체계 추적, 조문 비교, 계약/AGB 리스크 스크리닝, NDA triage, 종합 리서치 체인, 교차참조 추출, 품질 검증, 주법(Landesrecht), 개정 이력, 법률 용어 사전, 리스크 조기 경고, 소스 상태 점검, BMF-Schreiben(연방재무부 행정해석)까지 커버합니다.
...
| 도구 | 설명 |
|------|------|
| `search_law` | 키워드/일상어로 연방법령 검색 (query expansion 지원) |
| `get_law_section` | 특정 조문 전문 조회 (`§ 437 BGB` 등) |
| `get_law_toc` | 법률 목차·체계 조회 |
| `search_case_law` | 연방법원 판례 검색 (법원별 필터, query expansion 지원) |
...
| `get_state_law_section` | 주법 조문 조회 — Bayern/NRW 실시간 파싱, 기타 주 URL 안내 |
